...
PREFIX ?= /usr/local
BINDIR ?= $(PREFIX)/bin
COMPLETION_ZSH_DIR ?= /usr/share/zsh/site-functions
COMPLETION_BASH_DIR ?= /usr/share/bash-completions
COMPLETION_ZSH ?= $(COMPLETION_ZSH_DIR)/_toolen
COMPLETION_BASH ?= $(COMPLETION_BASH_DIR)/toolen
...
install: $(OUTPUT) install.completions
	$(Q)install -d $(DESTDIR)$(BINDIR)
	$(Q)install -m 755 $(OUTPUT) $(DESTDIR)$(BINDIR)/
	$(Q)printf "Toolen was installed on your system\n"

install.completions:
	$(Q)[ -d "$(COMPLETION_ZSH_DIR)" ] && install -m 666 completions/_toolen $(COMPLETION_ZSH_DIR)/
	$(Q)if [ -d "$(COMPLETION_BASH_DIR)" ]; then \
		cp completions/toolen.bash completions/toolen; \
		install -m 666 completions/toolen $(COMPLETION_BASH_DIR)/; \
		rm -f completions/toolen; \
	fi

uninstall:
	$(Q)rm -f $(DESTDIR)$(BINDIR)/$(OUTPUT)
	$(Q)rm -f $(COMPLETION_ZSH)
	$(Q)rm -f $(COMPLETION_BASH)
	$(Q)printf "Toolen was removed on your system\n"
...
help:
	...
	$(Q)echo "  install             install toolen to system"
	$(Q)echo "  install.completions install completions only"
	$(Q)echo "  uninstall           remove toolen from the system"
